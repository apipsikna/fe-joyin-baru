...
export default function ReferralPro({ profile }) {
  ...
  // const { referralCode } = useReferralCode(profile); // ❌ NOT USED
      ...
      >
        ...
        <motion.div variants={fadeUp} className="relative z-10">
          <h1 className="text-center text-white font-extrabold tracking-wide text-[32px] md:text-[44px] leading-tight">
            {t("referralPro.hero.title", { defaultValue: "Ajak Teman, Dapatkan Komisi" })}
          </h1>

          <p className="mt-3 text-center text-white/90 text-[13px] md:text-[15px] leading-relaxed max-w-3xl mx-auto">
            {t("referralPro.hero.desc", { defaultValue: "Ajak temanmu..." })}
          </p>
          ...
          >
            <div className="text-white/90 font-semibold text-[16px] md:text-[18px]">
              {t("referralPro.code.label", { defaultValue: "Kode Referral Anda :" })}
            </div>
            ...
            <div className="mt-3 flex items-center gap-3">
              <input
                value={referralCode}
                readOnly
                placeholder={t("referralPro.code.placeholder", { defaultValue: "Masukkan kode referral" })}
              ...
              <button
                ...
                title={t("referralPro.code.share", { defaultValue: "Share" })}
              >
                <HiOutlineDocumentDuplicate className="w-5 h-5" />
                {copied ? t("referralPro.code.copied", { defaultValue: "Tersalin" }) : t("referralPro.code.copy", { defaultValue: "Salin" })}
              </button>
            </div>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          >
            <div className="mx-auto w-full max-w-6xl">
              ...
              >
                {t("referralPro.list.title", { defaultValue: "Daftar Referral" })}
              </motion.h2>
                  ...
                  <thead className="bg-[#5FCAAC] text-white">
                    <tr>
                      <th className="py-4 px-4 text-left font-bold w-[64px]">
                        {t("referralPro.list.table.no", { defaultValue: "No" })}
                      </th>
                      <th className="py-4 px-4 text-center font-bold">{t("referralPro.list.table.name", { defaultValue: "Nama" })}</th>
                      <th className="py-4 px-4 text-center font-bold">{t("referralPro.list.table.email", { defaultValue: "Email" })}</th>
                      <th className="py-4 px-4 text-center font-bold">{t("referralPro.list.table.time", { defaultValue: "Waktu" })}</th>
                      <th className="py-4 px-4 text-center font-bold w-[120px]">
                        {t("referralPro.list.table.status", { defaultValue: "Status" })}
                      </th>
                    </tr>
                  </thead>
