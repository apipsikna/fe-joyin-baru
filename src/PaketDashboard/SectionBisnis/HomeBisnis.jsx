...
export default function HomeBisnis({ profile }) {
...
function WelcomeSection({ profileName, t }) {
  ...
  return (
    <div className="flex justify-between items-center flex-wrap relative z-0 overflow-visible w-full px-6 md:px-12">
      ...
      >
        <style>{`
          .gelombang-dyn { width: ${px(g.w)}; }
          @media (min-width: 768px){
            .gelombang-dyn { width: ${px(g.wMd)}; }
          }
        `}</style>
        ...
        <img
          src={gelombang}
          alt="gelombang"
          className="gelombang-dyn absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none select-none z-0"
          style={{
            opacity: g.opacity,
  ...
  return (
    <div className="mt-12 bg-white rounded-3xl shadow-lg p-8 w-full">
      ...
      <div className="w-full rounded-2xl border border-emerald-100 shadow-[0_10px_30px_rgba(92,201,175,.15)] p-3">
        <div className="w-full h-[340px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 20, right: 24, left: 0, bottom: 10 }} barCategoryGap={18}>
              <defs>
              ...
              <Tooltip content={<CustomTooltip t={t} />} cursor={{ fill: "rgba(16,185,129,0.06)" }} />
              ...
              <ReferenceLine
                ...
                label={{
                  value: `${t("home.stats.average", { defaultValue: "Rata-rata" })}: ${avg}`,
                  position: "right",
                  fill: "#10b981",
                  fontSize: 12,
                  fontWeight: 600,
              ...
              <Bar dataKey="value" fill="url(#barGradient)" radius={[12, 12, 8, 8]} maxBarSize={38} isAnimationActive={false}>
                <LabelList
                  dataKey="value"
                  position="top"
                  formatter={(v) => `${v}`}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {t("home.stats.note", {
          defaultValue: "Data di atas merupakan contoh. Integrasikan dengan API analitik Anda untuk angka aktual.",
        })}
      </p>
    </div>
  );
  ...
  return (
    <div className="mt-8 bg-gray-50 border border-gray-200 rounded-2xl shadow-inner p-6">
      ...
      <ul className="space-y-3 text-gray-600">
        <li>✅ {t("home.services.item1", { defaultValue: "Menjawab pertanyaan pelanggan otomatis" })}</li>
        <li>✅ {t("home.services.item2", { defaultValue: "Memberikan panduan produk & layanan" })}</li>
        <li>✅ {t("home.services.item3", { defaultValue: "Mendukung kebutuhan pelanggan 24/7" })}</li>
        <li>✅ {t("home.services.item4", { defaultValue: "Statistik performa bot real-time" })}</li>
      </ul>
    </div>
  );
}
